...
func TestPackageSignatureFilesAndTamperDetection(t *testing.T) {
	...
	files := map[string][]byte{"ui/index.html": []byte("fixture UI"), "runtimes/linux-amd64/gpt-inspector": []byte("fixture binary")}
	if err := writePackage(path, files, map[string]runtimeEntry{"linux-amd64": {Path: "runtimes/linux-amd64/gpt-inspector"}}, key, "fixture-publisher"); err != nil {
		t.Fatal(err)
	}
