...
版本：0.1.9。日期：2026-09-25。

插件变更仅位于独立目录，不改 Sub2API 主程序或 GPT Inspector。本版审查并修正工具历史校验、多图上传状态和附件错误脱敏；保留固定扩展名、JPEG/PNG 探测、additional_tools、完整外部历史转换与图片独立诊断。默认允许模型保留 gpt-6-astra。

遵照用户“不要自测，直接打包，由用户自测”的要求：本次进行源码对照、代码修改、格式化、JS 语法检查、三平台编译、签名与包内容核验及打包；不运行 Go/Node/浏览器功能测试、真实账号探测或部署。补充的回归用例覆盖空 custom input、异常句柄前缀、错误 namespace、多图上传后的取消与错误脱敏、UI 缓存/失败状态区分；本次未执行。
...
6. 升级后确认记录插件为 v0.1.9 且实例号变化；先选 JPEG / auto 做图片路由探测，再选 PNG 对照。真实桌面拖图应显示附件 `image.jpg / image/jpeg`，新实例首次同图为上传。若仍为 got none，保留新的文件字段和 Responses 错误；本次不能预先认定远端附件元数据已修复。
...
升级复用原发布者公钥：停用旧版、导入 0.1.9、启用并检查账号路由与新实例号。已保存的模型列表继续保留。图片传输字段默认 attachment，passthrough 可对照旧行为；切换不会自动重试已发送请求。原 0.1.8 及更早产物保留，新包使用独立版本号。
