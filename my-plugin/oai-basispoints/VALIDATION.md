...
版本：0.1.11。日期：2026-09-25。

插件继续位于独立目录。本版新增清空全部路由与图片诊断历史、跨窗口选择重置、旧在途记录隔离、短诊断 ID 及未桥接工具类型提示。保留 0.1.10 工具链路修复；本轮没有修改宿主代码或 BPS 生图协议。默认模型保持不变，包含 gpt-6-astra，不新增 gpt-6-sol。

遵照用户“不要自测，直接打包，由用户自测”的要求：未运行 Go/Node/浏览器功能测试、真实账号探测、生图调用或部署。已完成源码复核、Go 格式化、JS 语法检查、git diff --check、Go 测试源码编译（go test -c，仅编译），以及 Linux amd64 / Linux arm64 / macOS arm64 插件编译与签名打包。54 个文件的签名/哈希/包内容与本地构建一致性校验通过；未打入私钥。清空按钮、多窗口、在途请求及 Windows 生图定位流程仍待用户实测。

0.1.11 安装包 SHA256：`6409c0bac18c96346663b11eb3c5cfc4b2e39689201e51fbda186929e91efb0c`。新增功能未改动生图后端，本包不能作为生图修复通过的证据。
...
0.1.10 安装包 SHA256：`764cd838062b6557dfd30db0d752e6d782917d58b6dcfe98a2089dbe775de588`。旧发行 ZIP 附有独立宿主补丁，基于工作区提交 `089fe4ea1`。0.1.11 发行包只含插件产物与文档，不重复附带旧宿主补丁。
...
- 用户表示旧版本缺少删除历史按钮，无法识别 Windows 请求。本版补上清空入口；不借此声称已经实现生图。
...
6. 升级后确认记录插件为 v0.1.11 且实例号变化。输入图片已获用户反馈通过，本轮没有再次发送图片请求；本次重点核对生图工具声明。
...
升级复用原发布者公钥：停用旧版、导入 0.1.11、启用并检查账号路由与新实例号。已保存的模型列表继续保留。图片传输字段默认 attachment，passthrough 可对照旧行为；切换不会自动重试已发送请求。旧产物保留，新包使用独立版本号。宿主源码改动需另行构建部署，插件升级不会自动修改服务器主程序。
