...
func (d *requestDiagnostic) retry(reason string) {
	if len(d.ResponseRetries) < 16 {
		d.ResponseRetries = append(d.ResponseRetries, responseRetryDiagnostic{
			Attempt: d.ResponseAttempts, Reason: reason, HTTPStatus: d.HTTPStatus,
			RequestID: d.RequestID, UpstreamError: d.UpstreamError, UpstreamErrorState: d.UpstreamErrorState,
		})
	}
}
