# ranxi2001/sub2api 对照与路由边界（0.1.21）

## 0.1.21 当前宿主的 WebSocket 桥接

本版另核对了本地宿主 HEAD `1f1b9966f`；用户确认该源码对应线上版本。`openai_ws_forwarder_ingress.go` 的 `forceHTTPBridge` 在账号命中 `PluginManager.ShouldRouteOpenAIOAuth` 时自动启用，桥接经 `doOpenAIUpstream` 调用现有插件。`openai_ws_http_bridge.go` 对 `type=keepalive` 在首输出前单独放行，不释放暂存生命周期事件，也不把心跳记作语义输出。插件 0.1.21 据此发送独立心跳，不修改宿主、不增加监听端口。客户端必须实际通过 WebSocket 进入这条链路；插件无法从现有 transport 接口证明客户端握手或宿主刷新。完整配置、验证和回滚见 [WEBSOCKET_KEEPALIVE.md](WEBSOCKET_KEEPALIVE.md)。

以下原生 Excel 对照保留其当时的固定参考提交，不能与上面的本地宿主核对混为同一次版本比较。
...
1. 先确认入口；插件路径升级后应产生 v0.1.21 新记录。原生路径没有新插件记录可能正常，勿用旧记录定位新请求。
